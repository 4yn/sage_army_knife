...
# This file was *autogenerated* from the file /home/sy/ctf2/sage_army_knife/sage_army_knife/script_bag/pqlx_ec_check_param.sage
from sage.all_cmdline import *   # import sage library

_sage_const_2 = Integer(2); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1)# pqlx_ec_check_param.sage
...
def check_pohlig_hellman(curve, generator=None):
    ...
    naive_complexity = order.nth_root(_sage_const_2 , True)[_sage_const_0 ] + _sage_const_1 

    # for an order N = (p_0)^(e_0) * (p_1)^(e_1) * ... * (p_k)^(e^k) with p prime and e natural the complexity is:
    # O( \sum_{i=0}^k e_i ( log_2(n) + sqrt((p_i)) ) )

    logn = log(order, _sage_const_2 )

    pohlig_hellman_complexity  = sum( y * (logn + x.nth_root(_sage_const_2 , True)[_sage_const_0 ]) for x, y in factorization)
...
def check_smart(curve):
    ...
    return curve.trace_of_frobenius() == _sage_const_1 
...
def check_mov(curve):
    ...
    return curve.trace_of_frobenius() == _sage_const_0 
...
def check(curve, generator=None):
    ...
    if ph[_sage_const_0 ] < ph[_sage_const_1 ]:
        quotient = round( float(ph[_sage_const_1 ]/ph[_sage_const_0 ]), _sage_const_2 ) - _sage_const_1 

        logs = [round(float(log(x, _sage_const_2 )), _sage_const_2 ) for x in ph]

        print_red(f"Pohlig-Hellman can make ECDLP solving {quotient} times faster!")
        print(f"O(2^{logs[_sage_const_1 ]}) -> O(2^{logs[_sage_const_0 ]})")
