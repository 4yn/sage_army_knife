...
# This file was *autogenerated* from the file /home/sy/ctf2/sage_army_knife/src/sage_army_knife/dot_sage/rsa.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_1000 = Integer(1000)
...
class RSAKnife:
    def __init__(
        ...
        if multiprime is not None:
            ...
            if isinstance(multiprime[_sage_const_0 ], (tuple, list)):
                assert all([len(i) == _sage_const_2  for i in multiprime])
            else:
    ...
    def pqphi(self):
        if self.p is not None and self.q is not None and (self.phi is None):
            self.phi = (self.p-_sage_const_1 ) * (self.q-_sage_const_1 )
            assert self.phi == (self.p-_sage_const_1 ) * (self.q-_sage_const_1 )

        if self.p is not None and (self.q is None) and self.phi is not None :
            self.q = (self.phi // (self.p-_sage_const_1 )) + _sage_const_1 
            assert self.phi == (self.p-_sage_const_1 ) * (self.q-_sage_const_1 )
        
        if (self.p is None) and self.q is not None and self.phi is not None:
            self.p = (self.n // (self.q-_sage_const_1 )) + _sage_const_1 
            assert self.phi == (self.p-_sage_const_1 ) * (self.q-_sage_const_1 )
    ...
    def ed_from_phi(self):
        ...
        if self.e is not None and self.d is None:
            self.d = ZZ(Zmod(self.phi)(self.e) ** -_sage_const_1 )
            assert (self.e * self.d) % self.phi == _sage_const_1 

        if self.e is None and self.d is not None:
            self.e = ZZ(Zmod(self.phi)(self.d) ** -_sage_const_1 )
            assert (self.e * self.d) % self.phi == _sage_const_1 

        return self

    def pq_from_ed(self, iters=_sage_const_1000 ):
        ...
        for _ in range(iters):
            ...
            y = Fn(g) ** r
            if y == _sage_const_1  or y == Fn(-_sage_const_1 ):
                continue

            for j in range(_sage_const_1 , t):
                x = y ** _sage_const_2 

                if x == _sage_const_1 :
                    self.p = ZZ(gcd(y-_sage_const_1 , n))
                    self.q = n // self.p
                    return self
                
                if x == Fn(-_sage_const_1 ):
                    break
